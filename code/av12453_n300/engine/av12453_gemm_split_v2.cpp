...
//       untouched, so the result is bit-identical to the serial version.
//       New timers: empty-stack wall AND process CPU time.
//   (2) CERTIFY is hardened: a --truth file with fewer than min(N,150)+1 usable
//       entries is a hard error (exit 2) instead of a vacuous pass, and it is
//       diagnosed before the transfer starts, not after it.
//       -DNO_PARALLEL_EXTRACT rebuilds the serial v1 loop (timing control).
//   (3) built with -Wall -Wextra (warning-free).
//   (4) -DUSE_CBLAS is exercised against a reference cblas_dgemm (cblas_stub/).
//
...
// process-wide CPU time (all threads), used to time the empty-stack phase in a
// way that is meaningful while other jobs share the machine.
static inline double cpu_now() {
    struct timespec ts;
...
#pragma omp parallel for schedule(static) num_threads(nthreads) reduction(+ : ac)
    ...
    //
    // G[p][q] is the literal completion count H_{(p,q)} of the empty-stack phase
    // (paper eq:G).  The cell (p,q) of mass = p+q reads only
    //   G[h][mass-h-1]          (mass-1),
    //   G[p][h], h < q          (p+h = mass-1),
    //   G[c][s], c <= p, s < slen(p-c,h), h <= q-1, delta = q-1-h >= 1,
    // and in the last family c + s <= c + slen(p-c,h) - 1 <= p + h <= mass - 1
    // (for c = p, i.e. aa = 0) resp. <= p + h - 1 <= mass - 2 (for aa >= 1).
    // So EVERY read is of a cell of strictly smaller mass: the mass+1 cells of a
    // fixed mass are mutually independent and can be computed in parallel.
    // Each cell keeps its own serial accumulation order, so the u64/Barrett
    // arithmetic per cell is untouched and the result is bit-identical to the
    // serial v1 loop for any thread count.
    std::vector<u32> terms() {
        ...
        for (int mass = 1; mass <= N; ++mass) {
            // schedule(dynamic,1): the work of cell (p,q) is ~ p*q*mass/2, so the
            // cells of one mass differ by orders of magnitude in cost.
            // -DNO_PARALLEL_EXTRACT rebuilds the v1 serial loop with the v2
            // instrumentation, as the control for the before/after timing.
#if defined(_OPENMP) && !defined(NO_PARALLEL_EXTRACT)
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads) reduction(+ : gm)
#endif
...
static std::vector<u32> load_truth_mod(int N, const char* path) {
    ...
    while (fgets(line, sizeof line, f)) {
        ...
        if (*s == '#' || *s == '\n' || *s == '\r' || *s == 0) continue;
        char* after_n = s;
        long n = strtol(s, &after_n, 10);
        if (after_n == s) {
            fprintf(stderr, "truth file %s line %ld: no degree field\n", path, lineno);
            fclose(f); exit(2);
        }
        s = after_n;
        while (*s == ' ' || *s == '\t') ++s;
        if (!(*s >= '0' && *s <= '9')) {
            fprintf(stderr, "truth file %s line %ld: no value field for n=%ld\n", path, lineno, n);
            fclose(f); exit(2);
        }
        u64 v = 0;
        ...
        if (n > N) continue;
        if ((long)out.size() != n) {
            fprintf(stderr, "truth file %s line %ld: degree %ld out of order (expected %ld)\n",
                    path, lineno, n, (long)out.size());
            fclose(f); exit(2);
        }
        out.push_back((u32)v);
    }
...
// v2: the CERTIFY check is only meaningful if the truth file really covers
// n = 0 .. min(N,150).  A short (or empty) file used to yield "CERTIFY: OK ...
// for 0 <= n <= -1"; it is now a hard error.
static int truth_required_entries(int N) { return (N < 150 ? N : 150) + 1; }
static double now() {
    using namespace std::chrono;
    return duration<double>(steady_clock::now().time_since_epoch()).count();
}
...
int main(int argc, char** argv) {
    ...
    // ---- v2: validate the truth file BEFORE the (possibly hours long) transfer.
    std::vector<u32> tr = load_truth_mod(N, truth);
    const int need = truth_required_entries(N);
    if ((int)tr.size() < need) {
        printf("CERTIFY: FAIL  truth file %s supplies %d entries, need %d (n = 0..%d)\n",
               truth, (int)tr.size(), need, need - 1);
        fprintf(stderr,
                "truth file %s supplies only %d usable entries; CERTIFY needs n = 0..%d.\n"
                "Refusing to run: a short or empty truth file would make CERTIFY vacuous.\n",
                truth, (int)tr.size(), need - 1);
        return 2;
    }
    printf("# truth file %s: %d entries, CERTIFY will check n = 0..%d\n",
           truth, (int)tr.size(), need - 1);
    ...
    double t2 = now();
    eng.finalize_tables();
    const double xw0 = now(), xc0 = cpu_now();
    std::vector<u32> terms = eng.terms();
    const double extract_wall = now() - xw0, extract_cpu = cpu_now() - xc0;
    double t3 = now();
    ...
    printf("# empty-stack phase: wall %.3fs  cpu %.3fs  (threads = %d, speed-up %.2fx)\n",
           extract_wall, extract_cpu, threads,
           extract_wall > 0 ? extract_cpu / extract_wall : 0.0);
    printf("# whole run: wall %.3fs  cpu %.3fs\n", t3 - t0, cpu_now());
    ...
    if (sidecar) {
        ...
        fprintf(f, "  \"empty_stack_wall_s\": %.3f,\n", extract_wall);
        fprintf(f, "  \"empty_stack_cpu_s\": %.3f,\n", extract_cpu);
        fprintf(f, "  \"total_cpu_s\": %.3f,\n", cpu_now());
